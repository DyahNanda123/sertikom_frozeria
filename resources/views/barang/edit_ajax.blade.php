<form action="{{ route('barang.update', $barang->id) }}" method="POST" class="form-ajax" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    ...
    <div class="row">
        <div class="col-md-6 form-group">
            ...
            <input type="text" name="nama_barang" class="form-control" required value="{{ $barang->nama_barang }}" placeholder="Contoh: Ayam Nugget Crispy">
        </div>
        <div class="col-md-6 form-group">
    ...
    <div class="form-group">
        ...
        <textarea name="deskripsi" class="form-control" rows="3" placeholder="Tuliskan keterangan detail mengenai produk ini...">{{ $barang->deskripsi }}</textarea>
    </div>

    <div class="form-group mb-4">
        <label class="text-muted font-weight-bold">Foto Produk</label>
        @if($barang->foto_barang)
            <div class="mb-2">
                <small class="text-primary d-block mb-1 font-weight-bold">Foto Saat Ini:</small>
                <img src="{{ asset('storage/barang/'.$barang->foto_barang) }}" class="img-thumbnail" style="height: 80px; border-radius: 8px;">
            </div>
        @endif
        <div class="custom-file">
            <input type="file" name="foto_barang" class="custom-file-input" id="customFileEdit" accept="image/*">
            <label class="custom-file-label" for="customFileEdit">Ganti file foto (opsional)...</label>
        </div>
    </div>
    ...
    <div class="row">
        ...
        <div class="col-md-4 form-group">
            ...
            <input type="number" name="stok_minimum" class="form-control" required value="{{ $barang->stok_minimum }}">
        </div>
        <div class="col-md-4 form-group">
            <label class="text-muted font-weight-bold">Satuan <span class="text-danger">*</span></label>
            <input type="text" name="satuan" class="form-control" required value="{{ $barang->satuan }}" placeholder="Pcs / Pack">
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-md-6 form-group">
            ...
            <input type="text" name="berat_ukuran" class="form-control" value="{{ $barang->berat_ukuran }}" placeholder="Contoh: 500 gram">
        </div>
        <div class="col-md-6 form-group">
            <label class="text-muted font-weight-bold">Lokasi Simpan</label>
            <input type="text" name="lokasi_simpan" class="form-control" value="{{ $barang->lokasi_simpan }}" placeholder="Contoh: Freezer A1">
        </div>
    </div>
